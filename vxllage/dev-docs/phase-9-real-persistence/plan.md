...
VXLLAGE's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: posts, follows, villages, villageEvents)
...
Created a real post, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8796) no longer loses any real state
