...
VSAFE's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: safetyCheckIns, idVerifications, callSessions)
...
Recorded a real safety check-in, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8799) no longer loses any real state
