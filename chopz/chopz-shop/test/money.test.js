...
test('an order splits three ways and adds back up to what the buyer paid', async () => {
  ...
  const order = await orders.createOrder(store, {
    buyerId: 'sam', productId: product.id, affiliateLinkId: link.id, settleFn, now: NOW,
  });
  ...
  assert.strictEqual(settleFn.of('sam'), 400);
  assert.ok(settleFn.isDrained(orders.CHOPZ_ESCROW_ACCOUNT),
...
test('apparel is charged the apparel fee, not the general one', async () => {
  ...
  const mugOrder = await orders.createOrder(store, {
    buyerId: 'sam', productId: mug.id, settleFn, now: NOW,
  });
...
test('a product with no category falls back to the general fee', async () => {
  ...
  const order = await orders.createOrder(store, {
    buyerId: 'sam', productId: product.id, settleFn, now: NOW,
  });
...
test('a failed charge leaves no order behind', async () => {
  ...
  await assert.rejects(() => orders.createOrder(store, {
    buyerId: 'broke', productId: product.id, settleFn: declining, now: NOW,
  }), /insufficient funds/);
...
test('a real click earns the commission without the buyer naming the link', async () => {
  ...
  const order = await orders.createOrder(store, {
    buyerId: 'sam', productId: product.id, settleFn, now: NOW + HOUR,
  });

  assert.strictEqual(order.affiliateLinkId, link.id);
  assert.ok(order.affiliateCommission > 0);
  assert.strictEqual(settleFn.of('kaya'), order.affiliateCommission);
...
test('one buyer’s click does not earn commission on another buyer’s order', async () => {
  ...
  const order = await orders.createOrder(store, {
    buyerId: 'ada', productId: product.id, settleFn, now: NOW + HOUR,
  });
  assert.strictEqual(order.affiliateCommission, 0);
  assert.strictEqual(settleFn.of('kaya'), 0);
});
...
test('a link generated for another product cannot be claimed on this one', async () => {
  ...
  await assert.rejects(() => orders.createOrder(store, {
    buyerId: 'sam', productId: tee.id, affiliateLinkId: link.id, settleFn, now: NOW,
  }), /different product/);
  assert.strictEqual(settleFn.of('sam'), 500, 'a refused order must not charge the buyer');
});
...
test('a cart checkout charges every item and settles each one', async () => {
  ...
  assert.strictEqual(settleFn.of('sam'), 900);
  ...
  assert.ok(settleFn.of('maker') > 0);
  assert.ok(settleFn.of('other') > 0);
  assert.ok(settleFn.isDrained(orders.CHOPZ_ESCROW_ACCOUNT));
  assert.ok(Math.abs(settleFn.drift()) < 0.01);
});
...
test('cart items settle sequentially, so a later failure cannot un-pay an earlier seller', async () => {
  ...
  // buyer charge. Each order is a single atomic settlement now, so the
  // second *call* is the second item — the test's intent is unchanged
  // and it is the count that had to move.
  let calls = 0;
  const flaky = async (legs) => {
    calls += 1;
    if (calls > 1) throw new Error('insufficient funds');
    return { ok: true };
  };
  ...
  // per-item settlement, and it is asserted rather than assumed: the
  // alternative — a partial rollback that un-pays a seller who was
  // already paid — is worse and harder to detect.
  //
  // Deliberately per-item rather than one settlement for the whole
  // cart: one seller's shortfall must not refuse everybody's order, and
  // each completed item is already recorded, so a retry resumes rather
  // than repeats. What must hold is that a *single order's* four legs
  // move together, which the tests above assert.
