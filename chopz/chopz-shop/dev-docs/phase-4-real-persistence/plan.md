...
CHOPZ SHOP's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: products, affiliateLinks, orders)
...
Created a real product listing, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8801) no longer loses any real state
