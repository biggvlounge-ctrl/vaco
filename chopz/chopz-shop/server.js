...
app.post('/chopz-shop/orders', requireActor('buyerId'), async (req, res) => {
  try {
    res.status(201).json(await createOrder(store, { ...req.body, transferFn: transferVCoin }));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/chopz-shop/cart-checkout', requireActor('buyerId'), async (req, res) => {
  try {
    const checkout = await createCartCheckout(store, { ...req.body, transferFn: transferVCoin });
