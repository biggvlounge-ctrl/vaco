...
CHOPZ's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: videos)
...
Uploaded a real video, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8800) no longer loses any real state
