...
Shield's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: sessions)
...
Logged in via a real Shield session, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8812) no longer loses any real state
