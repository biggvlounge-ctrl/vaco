...
VOID MAGIC's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: experiences, bookings, eventServiceRequests)
...
Created a real bookable experience, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8797) no longer loses any real state
