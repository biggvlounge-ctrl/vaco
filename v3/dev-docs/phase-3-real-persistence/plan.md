...
V3's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: vcoinBalances, vashBalances, transactions)
...
Transferred real VCoin between two users, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8811) no longer loses any real state
