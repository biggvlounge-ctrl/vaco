...
//
// **Why this exists before any crypto is actually used**: VACA had no
// cryptographic layer at all when this was written -- it records real
// verification decisions but never actually verified a signature.
// That is precisely why this was cheap: there was nothing hardcoded
// to retrofit. The rule this establishes: nothing in VACA ever calls
// a crypto primitive directly. Everything goes through `signPayload`/`verifySignature`/
...
// VACA is blockchain identity and "relies on quantum-vulnerable
// public-key signatures." An identity signature that can be forged
// retroactively is a different and arguably worse failure than
// decrypted data -- it lets an attacker impersonate a verified
// identity, not merely read it.
//
...
const HASH_ALGORITHMS = {
  ...
  'sha3-512': { available: true, quantumEffectiveBits: 256, note: 'Higher margin for long-lived identity records.' },
};
...
function getActiveSignatureAlgorithm() {
  return process.env.VACA_SIGNATURE_ALGORITHM || DEFAULT_SIGNATURE_ALGORITHM;
}

function getActiveHashAlgorithm() {
  return process.env.VACA_HASH_ALGORITHM || DEFAULT_HASH_ALGORITHM;
}
