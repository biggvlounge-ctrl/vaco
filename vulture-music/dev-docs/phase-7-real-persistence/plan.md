...
Vvltvre Music's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: releases, revenueReports, managementDeals)
...
Created a real release, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8806) no longer loses any real state
