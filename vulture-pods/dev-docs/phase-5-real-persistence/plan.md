...
Vvltvre Pods's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: shows, episodes, listenEvents)
...
Created a real podcast show, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8810) no longer loses any real state
