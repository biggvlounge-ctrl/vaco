...
### 2a. Current — 23 Sep 2026, read out of the tree

**Read this one.** §2b below is the original baseline and is kept as a
dated record rather than a description of the engine; every figure in
it was true at commit `b6b7a08` and almost none of it is true now.
Leaving it as the first thing a reader met was this project's
twenty-first standing rule in its own working document — a section
that goes stale in exactly one direction, because nothing prompts you
to re-read the thing that was accurate when you wrote it.

| Layer | State |
|---|---|
| Simulation engine | **76 modules, 1,688 KB** under `server/` |
| HTTP API | **81 routes registered** in `server.js` |
| PostgreSQL | **Durable.** `persistence.js` loads before `app.listen` and checkpoints every 10 ticks; `migrate.js` writes and `restore.js` reads back |
| Tests | **1,233 in vacon-c** across 69 files, plus 172 in `world-layer/` |
| Measured completeness | **91.4%** — `dev-docs/GAME_COMPLETENESS.md`, regenerated by `scripts/completeness.mjs` |
| World data | 24 sources registered, **22 with importers**; every host blocked at this environment's proxy, so nothing has been imported |

Sourced, per this file's own rule: module and route counts are counted
from the tree by `test/inventory.test.js`, which fails when this
disagrees with `server/`; the percent is held to the code by
`test/completeness.test.js`; the source counts come from
`world-layer/sources.js`.

### 2b. The original baseline — as of commit `b6b7a08`, HISTORICAL

Read out of the code at the time, not from a status document. Full
detail in `VACANCY_INVENTORY.md`.
...
**The prediction this made, and how it turned out** — kept because a
forecast is only worth anything next to its outcome. It said the shape
of the remaining work was "unusual and favourable": most of the 52
unbuilt endpoints already had their logic written and simply had no
route in front of them, so the work was routing rather than simulation,
"the cheapest kind of remaining work there is".

**That was half right and the wrong half was the expensive one.** The
routes did go in cheaply — 6 to 79. But almost every system behind them
turned out to be built, tested, green and *reached by nobody*: the
Behavior Engine's three tables had zero lines of code, `contest.js` had
never been called by any generated world, `hireEntity` had exactly one
caller, no child could ever be educated, and no social event had ever
reached the event log. What looked like routing work was routing on top
of systems that had never run. CLAUDE.md's eleventh standing rule is
the generalisation, and it was learned here.
