...
three entries in the forty with a bare `absent` and no note under it,
and the reason turned out to be that nobody had looked in the right
document. `VACANCY_TRAIT_DATABASE_ATTACHMENT.md` defines **five**
tier-level trait sheets — FAMILY, ORGANIZATION, CITY, CIVILIZATION and
CULTURE — and the engine had built four. `tourism` is one of twelve
CITY dimensions; `military`, `healthcare`, `education` and `security`
are four of twenty-one CIVILIZATION ones. Thirty-three named dimensions
across the two sheets, generated by nothing and read by nothing, and
§49 CITY DNA names "tourism city" and "military city" among the nine
identities a city can have.

`vacon-c/server/tierTraits.js` is the reconciliation: of the
thirty-three, five are stored and twenty-eight are already answered by
a real column, a computable rollup, another system, or the
Transportation deferral — each one named individually, because "we
checked, and this one is answered elsewhere" is exactly the knowledge
that evaporates. `vacon-c/server/statecraft.js` is the mechanism: one
budget, four competing priorities, and delivery **scaled by the mean
writ across each city's communities**, so the state funds its hospitals,
schools and stations where it governs and barely at all where it does
not. All three stop at `partial` for stated reasons — there are no
soldiers, there are no visitors, and healthcare reaches no outcome.
...
| § | State | Detail |
|---|---|---|
| 24 Knowledge recovery | **BUILT, 18 Sep 2026** | `server/knowledge.js`. The row this replaces was right that `entity_knowledge` models something different — information SPREAD, not knowledge-as-civilization-resource — and that distinction is what the new file is built on. Ten named sources, nine unlockable fields, and a generated world contains nine of the ten (`universities` needs a `research` organization no world founds). Reading moves the field's `skills` trait, the `educational` family and `npcs.education` for the self-taught. **The wire to "knowledge is a civilization resource" already existed**: `technology.learningOf` averages the `educational` family to lower the reemergence bar, and nothing had ever moved one, so that term was the population's birth draw forever. Measured over 100 ticks: 807 study sessions, mean educational trait 51.91 → 52.34. |
| 25 Education & knowledge tiers 1–10 | **PARTIAL, 18 Sep 2026** | The spec gives SEVEN tiers, not ten, and this row said ten — `occupations.KNOWLEDGE_TIERS` carries all seven with the spec's own subject lists, and every one of the 34 occupations sits on one. A source's tier against a reader's attainment is what decides whether studying it teaches anything. Still partial: no book LANGUAGES and no translation, which §25 does not mention but §60-odd does, and no prerequisite chain between tiers beyond attainment. |
...
| 32 Control node system | **PARTIAL, and the takeover half is built** | Territory control is computed and the four named node states (LOCKED / CONTESTED / CONTROLLED / FORTIFIED) are still not the implementation's vocabulary — `territory_blocks.status` has three of the four. **What IS built, 18 Sep 2026, is `server/control.js`**: `COMPOSITION_REQUIREMENTS_TRIBE_COHESION.md`'s Control Key across six scales from a one-bedroom apartment to a country, with the document's 5:10:1 composition used as a ratio against however many people hold the target now. |
