...
VACON's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: routingLog)
...
Routed a real agent request through the network, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8805) no longer loses any real state
