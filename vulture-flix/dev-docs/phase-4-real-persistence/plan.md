...
Vvltvre Flix's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: titles, watchEvents, subscriptions, streamSessions)
...
Added a real title, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8807) no longer loses any real state
