...
app.post('/api/subscriptions', requireActor('userId'), async (req, res) => {
  try {
    const sub = await subscribe(store, { ...req.body, transferFn: transferVCoin });
...
app.post('/api/titles', requireCallingService(), async (req, res) => {
  try {
    res.status(201).json(await acquireExclusiveTitle(store, { ...req.body, transferFn: transferVCoin }));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/api/titles/licensed', requireCallingService(), async (req, res) => {
  try {
    res.status(201).json(await licenseNonExclusiveTitle(store, { ...req.body, transferFn: transferVCoin }));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
