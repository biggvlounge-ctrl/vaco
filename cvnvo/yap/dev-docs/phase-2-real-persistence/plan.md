...
YAP's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: yapReports)
...
Submitted a real YAP safety report, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8802) no longer loses any real state
