...
VACO Analytics's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: events, alerts)
...
Ingested a real metric event, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8790) no longer loses any real state
