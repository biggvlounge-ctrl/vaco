...
HVNTZ's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: businesses, locations, revenueEvents)
...
Registered a real business, confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8792) no longer loses any real state
