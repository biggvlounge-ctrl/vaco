...
// direct VCoin transfer via an injected `settleFn`, the same
...
async function tipChannel(store, options = {}) {
  const {
    channelId, tipperId, recipientPersonId, amountVCoin, settleFn, now = Date.now(),
  } = options;
  ...
  if (typeof settleFn !== 'function') throw new Error('tipChannel requires a settleFn(legs, meta)');
  ...
  await settleFn(
    [{ fromUserId: tipperId, toUserId: recipientPersonId, amount: amountVCoin, reason: `vavlt_stvdios_tip:${channelId}:${tipId}` }],
    { reason: `vavlt_stvdios_tip:${channelId}:${tipId}` },
  );
