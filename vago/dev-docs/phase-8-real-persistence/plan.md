...
VAGO's own real business logic in `lib/*.js`.
...
a new top-level field (this store's own real fields today: goldCoinBalances, amoeEntries, casinoSessions, originalsRounds)
...
Submitted a real AMOE entry (Gold Coin grant), confirmed the resulting real data landed in
...
## Done when
A restart of this app (port 8795) no longer loses any real state
